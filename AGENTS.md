...
| Web presentation and navigation | Each page keeps much of its JS/CSS inline in `web/*.html`; shared `web/theme.*`, `mobile.*`, `transition.*` |
...
- Python dependencies: `pip install -r requirements.txt`. Create a local `.env` (ignored) with a strong `TEACHER_SIGNUP_CODE` of at least 12 characters plus Gemini, Pinecone, and Firebase configuration; see `README.md`. Run `waitress-serve --port=5000 app:app`.
- Offline Python checks are plain scripts: `python tests/test_profanity.py`, `test_security.py`, `test_stats_grouping.py`, `test_student_context.py`, `test_student_profile.py`, `test_quiz_answers.py`. Backend import tests need a valid throwaway teacher code in `.env`; external services are stubbed. Run relevant scripts after edits.
